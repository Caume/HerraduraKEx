/*  Herradura DSi (HDSI)- a Digital Signature scheme based on the FSCX function and 
...
/* Example build: gcc -DINTSZ=64 -o HDSI Herradura_DSi.c */
...
int main (){
  INT64 A,A2,B,B2,D,D2,FA,FA2,PSV,K,P,P2,S;
  srand(time(0));
  ...
  printf("\n\n--- Herradura Digital Signature- keys of different size & KEX done by Alice (HDSI) ---\n\n");
  PSV=FA;
  /*Here Alice performs KEX on its own, then publishes the entangled public key*/
  /*Alice's private (signing) key is: PSV,A,A2,B,PUBSIZE */
  printf("ALICE [ Private key = PSV,A,A2,B,PUBSIZE]:\n");
  printf("%llx PSV [Public from KEX]\n",PSV);
  printf("%llx A   [Secret from KEX]\n",A);
  printf("%llx A2  [Secret from KEX]\n",A2);
  printf("%llx B   [Secret from KEX]\n",B);
  printf("%llx P   [MSG to sign in plain text]\n",P);
  K=P^PSV^A^A2;
  S=FSCX_REVOLVE(&K,&B,PUBSIZE);
  printf("%llx S [Shared signed MSG, FSCX_REVOLVE(P xor PSV, B ,%u)] ->\n",S,PUBSIZE);
  
  /*Alice's public key is: B2,(INTSZ - PUBSIZE) */      
  printf("    BOB [ Alice's public key = B2,(INTSZ - PUBSIZE)]:\n");
  printf("    B2  %llx [Public from KEX]\n",B2);
  P2=(FSCX_REVOLVE(&S,&B2,INTSZ-PUBSIZE));  
  printf("    P2  %llx [signed MSG in plain text, FSCX_REVOLVE(S,B2,%u)] \n",P2,INTSZ-PUBSIZE);
  assert(P == P2);
