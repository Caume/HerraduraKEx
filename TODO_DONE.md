...
**All four languages now consume one pinned transcript, and all four have a
numbered prove-side test.**  The two halves are deliberate: a verify-side vector
cannot reach the PROVE path by construction, and a structural test cannot prove
two implementations agree on a byte layout.  Neither alone was enough.

**The design question the entry asked to settle first, settled as filed.**
`hcred_prove_kkw` draws one `os.urandom(32)` root per emulation, so a proof is
not a function of its statement and the other three KAT files' regenerate-and-
diff model cannot work.  `KAT/hcred_kkw.json` is therefore a VERIFY-SIDE vector:
captured once, checked in, consumed by everyone -- the route this entry
recommended, and the one that catches the bugs KKW actually had, since an
inverted aux-reveal condition, an under-allocated commitment buffer and a
flipped bit convention are all READER disagreements about a byte layout.  The
produce-side variant stays unbuilt and unneeded; it would put a test-only seam
in four production provers to buy strictly less.

**A four-language finding forced a two-set vector.**  HCRED's width is a runtime
argument in Python and Go (both demo at `n = 32`) but a COMPILE-TIME constant
fixed at 256 in C (`HCRED_N`) and Java (`Hcred.N`).  The four implementations
have never proved the same statement size, and two of them cannot run the size
the other two demo at.  So `n256` is the only width every language can consume;
`n32` is kept because one `n = 256` verification is ~70 s in Python, and the
full accept-plus-six-rejections matrix therefore runs there on `n32` and in the
compiled consumers on `n256` -- split by COST, not by coverage.

**C got a KAT verifier for the first time, via a generated header.**  C had none
of any kind -- Python checks its own vectors, `verify_kat.go` does Go,
`KatVerify` does Java -- in the language where two of the three port bugs were.
`KAT/verify_kat_c.c` consumes the vector through `KAT/hcred_kkw_vector.h`, the
transcript transposed into C arrays, so the dependency-free C tree needs no JSON
parser.  The header is a pure deterministic transform of the JSON, so unlike the
JSON it IS regenerate-and-diff checked: editing one without re-emitting the
other fails rather than drifting.

**And building it reproduced the bug class this item exists for.**  The first C
run reported `C REJECTS the pinned Python transcript`.  That was the consumer's
bug, not a wire split, and it was CHECKED rather than assumed -- comparing
`hcred_stmt_hash` across the two languages localised it to the syndrome, which C
stores in the reverse byte order of the big-endian integer Python and Go use
(`hcred_stmt_hash` reverses it back on the way in).  Same class as the
`hcred_kkw_outmap` endianness bug, and C-specific for the same reason:
`big.Int.Bit` and Python's `>>`/`&` are layout-independent where C byte arrays
are not.  The conversion now lives in the header emitter, once.

**Java needed a real JSON parser.**  `KatVerify`'s hand-rolled regex reader
handles flat `"key": "value"` objects only, and this file is nested (the online
proofs carry an array of path arrays), so the consumer brings a ~90-line
recursive-descent reader rather than a dependency.  Java's proof types are
immutable, so each tamper case REBUILDS the proof rather than poking a finished
one -- which is closer to what verification actually defends against.

**Numbered tests: [50] in C/Go/Python's shared numbering, [31] in Java's own.**
Accept-control plus the same six rejection axes the vector's tamper table
applies, so a divergence between test and vector is visible rather than two
independent opinions about what to check.  Unlike [46]-[49], the Python copy
calls the SUITE instead of re-implementing: KKW is ~113 lines of interlocking
cut-and-choose machinery and a second copy would be a new place for exactly the
divergence being guarded.  Verified against a deliberate break -- an
always-accept `hcred_verify_kkw` is caught on all six axes.

**What the vector happens to cover, worth recording because it was luck.**
`pbar = [3, 3, 2, 0]` at `N_par = 4` gives `has_aux = [false, false, true,
true]`, so both branches of `aux is revealed iff pbar != N_par-1` appear -- the
exact condition the Go port shipped inverted.  A capture that hit one branch
only would miss that bug entirely.  A future re-capture should check this still
holds rather than assume it.

**Result.**  C, Go, Python and Java all accept the same transcript and reject
all six tampered variants.  They were previously assumed to agree on the KKW
wire format; now they are known to.

Status: **DONE v6.0.2** — pinned verify-side vector consumed by all four languages (C via a generated header, its first KAT verifier of any kind), plus numbered prove-side tests [50]/[31].
...
**v6.1.0 — the manifest half is CLOSED, by census rather than by another read.**  The
remaining work above was "extend `PRIMITIVES` to the suite-internal (non-CLI) primitives
it does not yet name".  Extending it by hand would have been the ELEVENTH one-time read
of the source tree, which is the method this item's own acceptance criterion rules out
and the reason it survived eleven releases that each added to it.  So the manifest was
not extended by reading.  A fourth check now ENUMERATES the internal surface, and the
manifest is whatever it takes to make that check pass.

**Part 4, the internal-surface census.**  Per language: list the suite's own top-level
functions, subtract the ones that language's CLI calls, subtract the ones a `PRIMITIVES`
entry names, fail on the remainder.  The subtraction of CLI-called functions is this
item's own phrase "suite-internal (non-CLI)" made executable — it had been applied by
hand at every previous addition.  Each language is measured against ITS OWN CLI and ITS
OWN markers, so nothing has to normalise `nl_chi_v3_ba` / `NlChiV3` / `nl_chi_v3` /
`nlChiV3` into one name; the cross-language claim stays in `PRIMITIVES`' four cells,
written down rather than guessed by a matcher.  `CENSUS_EXEMPT` rules are
self-invalidating the way the manifest is: a rule matching nothing is an error, so a
family cannot leave its excuse behind for the next thing that matches it.

**27 entries -> 196; 108 markers -> 686.**  Not a selection — the census produced the
list.  34 entries carry an `acknowledged` reason, and most are decomposition
differences (C splits the KKW seed tree into expand/levels/node-idx where Go and Python
inline it; Java factors out `gateRound`/`issuerMsg`/`computeUpc`; Go and Python express
classical HSKE as `fscx_revolve` at the call site where C and Java name a wrapper).
Two are not:

* **`ratchet_erase` is C-only and cannot be otherwise.**  Go, Python and Java are
  garbage-collected, so superseded ratchet state cannot be reliably zeroized.  That is
  a real limit on `SecurityProofs-5.md` 11.8.3's forward-secrecy claim, now recorded
  where the parity check keeps it visible.
* **`hmac_hfscx_256` is absent from Java**, and its only consumer is Python-CLI-only
  (TODO #166's passphrase-encrypted PEMs) -> TODO #267.

**The finding that justifies the method.**  `rnl_contributory_kdf` —
`HFSCX-256(K_raw || n_A || n_B)`, TODO #89's RNG hardening, the step that turns the raw
Ring-LWR agreement into the session key — was in the SUITE in C and Java and only in the
CLI in Go and Python.  Same placement asymmetry v5.8.7 found for `rnl_validate_m_blind`,
one release later, one step more consequential: that one screens an input, this one
derives the key.  In two languages a suite-only caller (the path
`docs/examples/hello_herradura.py` demonstrates) could finish an HKEX-RNL exchange and
then derive the session key some other way.  Ported to both suites, byte-identical, both
CLIs now call the suite; no wire change (19/19, 4/4, 10/10, 3/3 across the RNL interop
scripts).  NO numbered test was added, unlike v5.8.7's [49] for the analogous move:
that one was a guard nothing exercised, so relocating it left it untested, whereas
every `hkex-rnl` CLI test in the repo runs through this KDF in all four languages and a
divergence fails key agreement outright.  What was missing here was REACHABILITY, not
coverage -- which is also why no behavioural test could have found it.

**A hole in the manifest itself, found the same way.**  `SUITE_FILES["java"]` was every
`.java` in the directory — `HerraduraCli.java`, `Codec.java` and three test drivers
included.  So the manifest had, from the other side, exactly the hole it exists to
catch: `rnl-validate-m-blind` had to be "deliberately anchored at the SUITE files" to
catch Python keeping a validator in its CLI, while a Java primitive found ONLY in
`HerraduraCli.java` would have scored as present.  `JAVA_NON_SUITE` separates the layers
in both directions.  Java markers may now be written `File.java::<regex>`, which pins
which class holds a primitive; scoping the census's coverage set the same way
immediately surfaced two functions the unscoped version had been vouching for
(`Herradura.rol`/`ror`, covered by the entry for `ZkpNl.rol` — a different function at a
different width).

**What this item does NOT close, now #267.**  The CLI FLAG surface.  This item's
CLI-surface half counted `--algo` tags, and all 29 dispatch in all four languages since
v6.0.0 — but `--passphrase`/`pkey --decrypt` is Python-only and `--kdf`/`--aead` differ
across all four, with Java recording its own gaps in a Javadoc sentence rather than
anywhere a check can read.  Filed as #267 rather than absorbed here: it is a different
surface with a different mechanism, and this item's three axes are all now met.

Status: **DONE v6.1.0** — the primitive manifest covers the whole internal surface in
all four languages (196 entries, 686 markers), and Part 4's census makes that
