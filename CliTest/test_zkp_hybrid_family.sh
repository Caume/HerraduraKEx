...
#
# The key is generated by PYTHON rather than per-language, and deliberately.
# Only Python and Java can produce one: C's genpkey has no --bits at all (an
# acknowledged cli_surface_gaps row -- the C suite is compiled for a single
# KEYBITS), and Go's defines the flag but ignores it for this algo, pinning
# n at ZkpNlDefaultN (TODO #281).  A four-way keygen row would therefore test
# those per-language scope decisions rather than the cap.  What is under test
# here is CONSUMPTION -- every CLI must read an n = 64 key and interoperate on
# proofs over it -- which is the direction the defect broke.
echo "── n = 64: the width where the four caps disagreed ──"
ZK64_PRIV="$TMP/zk64.pem"
if $CLI_PY genpkey --algo hpks-zkp-nl --bits 64 --out "$ZK64_PRIV" >/dev/null 2>&1; then
    ok "[py] genpkey --algo hpks-zkp-nl --bits 64"
else
    bad "[py] genpkey --algo hpks-zkp-nl --bits 64"
fi
