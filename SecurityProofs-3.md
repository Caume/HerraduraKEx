...
ZKP-NL at n=256 and R=219 yields 920 KB, which exceeds practical limits for most use cases.  The CLI defaults to n=8 (35.5 KB) for this reason.  The ZKB++ encoding (`zkp_nl_prove_pp`, implemented v1.9.81 — §11.10.6 direction 3) reduces the n=256 revolve proof to 464 KB ($1.98\times$, gate-broadcast-dominated — see §11.10.4 size breakdown), not the generic $5\times$/180 KB; the implemented single-step circuit drops to 31 KB ($5.5\times$, overhead-dominated).
