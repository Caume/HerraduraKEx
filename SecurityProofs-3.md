...
**Remaining #128 work.**  The transcript still ships all three parties' cleartext output shares (they must be bound into the Fiat-Shamir hash before the challenge; shipping only the unopened party's outputs is unsound because the verifier cannot reconstruct the opened parties' outputs before knowing the challenge).  The KKW preprocessing-model encoding is therefore the remaining size optimization, followed by the C/Go ports, CLI surface, and tutorial.  Demo parameters are $n = 32$, $R = 4$ ($R = 219$ for $2^{-128}$ soundness); the presentation nonce (`msg_bytes`) is bound into the statement hash, so transcripts do not replay across contexts.
