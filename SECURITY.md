...
| **HDRBG / FPE / tweakable-wide-block** (`rand`, `fpe`, `twk`) | Unclassified — no analysis exists | These three ship as CLI subcommands with no security analysis anywhere in this repository: no row here before TODO #238, no entry in `spec/`, no section in `SecurityProofs-*.md`. They are listed so their absence is visible rather than silent, and `spec/herradura-protocol-spec.json`'s `unfiled_cli_surface` enumerates them. **Treat them as unanalysed**: absence of a documented weakness here is absence of analysis, not evidence of strength | TODO #241 |
