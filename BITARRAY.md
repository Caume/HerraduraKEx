...
**Status: NORMATIVE for TODO #314.  C (pass 2, v9.1.0), Go (pass 3, v9.2.0) and Python
(pass 4, v9.3.0) conform; Java does not yet.**
...
variable, **silently discards the other operand**".  §6.1 and §6.2 are kept because the
defects are the argument for this document, and a fixed defect that nobody can still read
about is a fixed defect nobody learns from.
...
**C, Go and Python are in the gating set**, via `KAT/verify_bitarray_c.c`,
`KAT/verify_bitarray_go.go` and `KAT/verify_bitarray_py.py`, all run by
`CliTest/test_kat_vectors.sh`: 376/376 each.
...
| 5 | Java — the last, and the one whose own header concedes constant-time away *because of* `BigInteger` | open |
