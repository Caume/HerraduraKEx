...
**Status: NORMATIVE for TODO #314.  C conforms (pass 2, v9.1.0) and Go conforms
(pass 3, v9.2.0); Python and Java do not yet.**
...
**silently returns all zeros**" — §6.1 is kept because the defect is the argument for
this document, and a fixed defect that nobody can still read about is a fixed defect
nobody learns from.
...
**C and Go are in the gating set**, via `KAT/verify_bitarray_c.c` and
`KAT/verify_bitarray_go.go`, both run by `CliTest/test_kat_vectors.sh`: 376/376 each.
...
second, which is now**: two independent implementations against one pinned answer is what
no round-trip or interop test can supply, because those compare a port against another
port's opinion.  C reads the generated header and Go reads the JSON, so the two consumers
do not even share an input file.
...
| 4-5 | Python, Java, one per release, each joining the gating set | open |
...
other.  Note what pass 3 already changed and what it did not: Go's `RnlKdfSeed` now takes
the HIGH octets of the domain constant, so the truncation split TODO #313 found is
CLOSED IN THE CODE for C and Go — but `hske-nla1` is still refused below 256 bits in all
four CLIs, and stays refused until Python and Java land.
