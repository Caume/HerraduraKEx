...
This guide shows how to use the Herradura suite as a library in your own C, Go, or Python
project, and how to drive the same protocols from the CLI. Sections are organized **by
cryptographic function** — each protocol has its own section with CLI, C, Go, and Python
examples together, so you can compare how the same operation looks across languages and
verify that a session key, ciphertext, or signature really is identical everywhere.
...
library APIs. Set these up once, then jump to whichever protocol section you need.
...
`drbg_generate` in place of `fopen("/dev/urandom", "rb")` throughout — see
[HDRBG](#hdrbg-forward-secure-drbg).

Complete runnable example: [`docs/examples/c/hello_herradura.c`](examples/c/hello_herradura.c).
...
To vendor it into your own module, copy the `herradura/` directory into your project
and add or update your `go.mod`:
...
The classical protocols (HKEX-GF, HSKE, HPKS, HPKE) rest on the discrete logarithm
problem in GF(2^256)* and the linear FSCX primitive. They are vulnerable to quantum
attacks — see [Security notes](#security-notes) — but are the simplest entry point and
share key material across most of the other sections in this guide (the NL/PQC variants
below reuse the same key types).
...
See [Hash-based stateful signatures](#hash-based-stateful-signatures) — WOTS-F/XMSS-F are
a separate, hash-based key type, not a variant of HPKS.
...
```python
r_hpke  = h.BitArray.random(n)                                              # ephemeral scalar
R_hpke  = h.BitArray(n, h.gf_pow(h.GF_GEN, r_hpke.uint, poly, n))          # ephemeral pubkey
enc_key = h.BitArray(n, h.gf_pow(alice_pub.uint, r_hpke.uint, poly, n))    # enc key = C^r
ct      = h.fscx_revolve(plaintext, enc_key, h.I_VALUE)

dec_key = h.BitArray(n, h.gf_pow(R_hpke.uint, alice_priv.uint, poly, n))   # dec key = R^a
dec     = h.fscx_revolve(ct, dec_key, h.R_VALUE)
assert dec == plaintext
# Transmit R_hpke alongside ct; Alice decrypts with her private key.
```
...
The NL/PQC variants replace the linear FSCX revolve with NL-FSCX (a non-linear, harder to
invert construction) and add HKEX-RNL, a lattice-based (Ring-LWR) key exchange. HPKS-NL and
HPKE-NL still use the same GF(2^256)* key type as HPKS/HPKE — only the symmetric
sub-protocol is hardened; they are not fully post-quantum. See
[Security notes](#nlpqc-protocols-hske-nl-a1a2-hkex-rnl-hpks-nl-hpke-nl) for details.

### HPKS-NL Schnorr signature
...
### HSKE-NL-A1 counter-mode encryption
...
```bash
$CLI enc --algo hske-nla1 --key alice_sk.pem --in msg.bin --out ct.pem
$CLI dec --algo hske-nla1 --key alice_sk.pem --in ct.pem  --out recovered.bin
```
...
### HSKE-NL-A2 symmetric encryption
...
```bash
$CLI enc --algo hske-nla2 --key alice_sk.pem --in msg.bin --out ct.pem
$CLI dec --algo hske-nla2 --key alice_sk.pem --in ct.pem  --out recovered.bin
```
...
random nonce must be generated for every encryption; the tag authenticates both
the ciphertext and the AD. Decryption is verify-then-decrypt and fails if the
tag does not match.
...
Returns `(nonce, ct, tag)` on encrypt; returns the plaintext or `None` on decrypt.
...
Stern identification protocol (a zero-knowledge proof for syndrome decoding), used
both as a Fiat-Shamir signature (HPKS-Stern-F) and as a Niederreiter-style KEM
(HPKE-Stern-F). Parameters: C/Go/Python use N=n=256, t=16, rounds=32; assembly/Arduino
use N=32, t=2, rounds=4.
...
this is the syndrome decoding problem. The demo APIs take the plaintext error
vector directly; **production requires a QC-MDPC or similar decoder** to
recover `e'` from the syndrome.
...
`hpke_stern_f_decap(ct, 0, seed)` attempts brute-force but refuses if `C(n, t) > 2^32`.
...
`hpks-xmss` is the many-time wrapper (library API only) that manages a tree of
WOTS leaves.

> **Statefulness warning:** A WOTS-F leaf key must **never** be reused. Sign
> only one message per leaf index — reuse catastrophically breaks security.
> XMSS-F is the recommended multi-signature variant — it tracks the Merkle tree
> so each signing call advances the leaf counter automatically. Signatures are
> byte-for-byte interoperable across the Python, C, and Go CLIs (`CliTest/test_wots.sh`).
...
`cred-prove`, `cred-verify` wrap this API (see CLI above).
...
types. See the CLI section above for the one-liner workflow.

---

## Hash primitive and DRBG

### HFSCX-256 hash and MAC

Merkle-Damgård hash built on NL-FSCX v1, returning 32 bytes. Pass an empty/`NULL`
IV for a bare digest; pass `iv = key XOR _HFSCX256_IV` for a keyed MAC.
...
Fast-key-erasure DRBG built on NL-FSCX v1, exposed as the `rand` CLI command. It is
a **deterministic** DRBG, *not* an OS entropy source: identical seed + personalization
+ byte count produce byte-identical output across the Python, C, and Go
implementations. Suitable for constrained targets where `/dev/urandom` is
unavailable, or for deterministic test vectors. Seed from a full-entropy source
(≥ 32 bytes recommended); reseed after at most `DRBG_MAX_BLOCKS` (2^20) output blocks.
...
### NumPy acceleration (Python only)
...
3, 5, 17, 257, 641). All OPRF functions are declared in `herradura.h`; the PEM
wire format for OPRF keys is identical across C, Go, and Python CLIs.
...
same flow via `pake-register` and `pake-demo` subcommands (see above).

---

## MCP server

`Mcp/herradura_mcp_server.py` exposes the Python CLI (`genpkey`, `pkey`, `kex`, `enc`,
`dec`, `sign`, `verify`, `dgst`) as MCP tools over stdio, so an agent can drive them as
tool calls instead of shelling out. It's a from-scratch, stdlib-only implementation of
the MCP stdio transport — no `mcp` SDK, no pip dependency, consistent with this
project's zero-external-dependency convention.

```bash
python3 Mcp/herradura_mcp_server.py          # run the server directly (stdio)
...
**Read `Mcp/README.md`'s trust-model section before wiring this into a real agent.**
In short: every tool operates only on file paths the caller explicitly supplies (no
default key directory, no implicit state between calls), private-key file *contents*
are never echoed back in a tool response (only success/failure and the output path),
and the server performs no network I/O — every call is one local subprocess invocation
of the already-tested CLI, so "the agent can call this server" is exactly as much
blast radius as "the agent can run the CLI directly."
