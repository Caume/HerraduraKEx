...
| `corank_linear_box_decision.py` | Should the suite act on §11.22.2's 126 -> 64 co-rank improvement? §11.23 (TODO #232): the circulant floor — exhaustively at `n = 8, 16`, no rotation-based step admitting agreement beats `2(i-1)`, which the classical `M` already achieves — the concrete `(n-1, 1)` box in the bit basis (3 word-ops, but 64 raw plaintext bits in the clear), its conjugate (co-rank 64, minimum leaked weight 75, ~128x cost and 8 KB of matrix), the HPKS challenge-entropy knock-on 130 -> 192, and the weight-4 functional `E[0]^E[2]^E[128]^E[130]` verified 300/300 against the shipped `fscx_revolve`. Verdict: deprecate |
