...
- *Degree preservation:* Theorem 13 is satisfied for prefix $k \geq 4$ provided $\mathrm{wt}(B_{0..k-1}) \geq 2$ **and** $B_{0..1} \neq 0$ (a keygen constraint, analogous to the existing $\mathrm{wt}(B) \geq 2$ requirement).  The second half of that condition was added by TODO #291, which replaced this section's 300-sample degree detector with an exact Mobius-transform computation: of the 176 values of $B$ at $n = 8, k = 4$ satisfying the first half alone, **16 have algebraic degree 1**, and they are exactly those with both of $B$'s two lowest bits clear -- the prefix carry chain never starts, so no AND term appears however many higher bits are set.  Confirmed exactly at $n = 16$.  Acceptance at $k = 4$ falls from $\approx 0.69$ to $\approx 0.63$.
...
> **Continued in Part 5 — §11.8.3–§11.8.10** (SecurityProofs-5.md): PQ Signature Options · HPKE-Stern-KEM
