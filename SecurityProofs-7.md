...
> - **Part 5 — §11.8.3–§11.8.9** (SecurityProofs-5.md): PQ Signature Options · HPKE-Stern-KEM
...
All three agreed byte-for-byte on the colliding value, which made this a property of the construction rather than a bug in one port.  Two separately-advertised primitives, reachable under one key, were therefore not independent: either subcommand decrypted the other's output.  A caller who believes they are using distinct primitives for distinct purposes — the only reason to ship both — did not get that.  The suite already had the machinery to prevent it: `hfscx_256_ds` (TODO #93) and `hmac_hfscx_256` both exist, and neither was used here.

**Fixed in v4.0.0 (TODO #242).**  Each primitive now derives its subkey as `HFSCX-256-DS(tag, len(key)_be8 ‖ key ‖ tweak)`, with tag `0x20` for `fpe` and `0x21` for `twk`.  `CliTest/test_fpe_twk.sh` holds the CLI-level regression across all three implementations — including that `twk --decrypt` no longer undoes `fpe --encrypt` on exactly the input above — and test `[46]` in the C, Go and Python harnesses holds the library-level one.  Wire-format breaking; see `MIGRATING.md` §8.
...
**§11.24.3 The tweak derivation.**  Three problems sit in the one line `HFSCX-256(key ‖ tweak)`.  The first is §11.24.1.  The second is that the concatenation boundary was unencoded, so `fpe(P, key=AB, ctx=C)` and `fpe(P, key=A, ctx=BC)` collided — and the CLI takes key width from the session-key PEM, so key length was never fixed by the interface either.  The correct pattern was already in use two hundred lines away: `drbg_seed` length-prefixes its entropy, and TODO #235's KEM work length-prefixes for exactly this reason.  v4.0.0 adopts it: an 8-byte big-endian key length now precedes the key.

The third points the other way and is worth stating in the same breath.  Neither subcommand calls `nl_v2_key_is_valid` on the derived subkey, which `SECURITY.md`'s HSKE-NL-A2 row says "all three CLIs reject it via" — true for A2, not true here.  But because `B` is a hash output rather than user-supplied, the degenerate class (about `2^-129` of the space) is unreachable by chance and cannot be steered without the key.  Deriving `B` is what makes it unreachable, so on this specific axis `fpe`/`twk` are in a *better* position than the A2 they inherit from.  It was a missing defence-in-depth check, not a live break.  v4.0.0 adds it anyway, as a rejection loop that rehashes until the subkey is non-degenerate — a backstop rather than a fix.
...
**Fixed in v4.0.0 (TODO #242).**  `fpe` and `twk` now run at `R_VALUE` = 192, matching HSKE-NL-A2, folded into the same wire-format break as the domain separation rather than spent as a second one.  The comparative argument above is what motivated it: the identical primitive sat on opposite sides of #214's line depending only on a step count nothing recorded a reason for.

One inconsistency to fix when #214 is next revisited: it frames "the deployed step count is n/4 = 64 rounds at n = 256", which was right for HFSCX-256's v1 compression and for `fpe`/`twk` before v4.0.0, and wrong for HSKE-NL-A2 at 192.
...
As originally filed: `fpe` **broken as named** — not format-preserving in the SP 800-38G sense (§11.24.2), the same function as `twk` at twelve-byte contexts (§11.24.1), and as a deterministic tweakable permutation A2 at one third the rounds (§11.24.4).  `twk` **demo-only** — the right shape for its stated purpose, with per-tweak determinism an expected XTS-style property rather than a defect, but the collision, the round count and the unencoded key boundary together more than a caveat.  `rand` **demo-only** and closest of the three to sound.

**After TODO #242 (v4.0.0)** the two fixable defects are closed and the verdicts move as follows.  `fpe` stays **broken as named**, unchanged and deliberately: #242 fixed the collision and the derivation but scoped the naming out, so the construction is now sound-shaped while still not being the thing it is named after.  `twk` stays **demo-only**, but for a different reason — all three of its listed blockers are closed, and what remains is the absence of a positive result rather than a defect list: no reduction to a standard tweakable-cipher definition (no STPRP argument), and NL-FSCX v2's security conjectural with only key-averaged trail bounds behind it.  Reclassifying it on the strength of "the defects found are fixed" would be exactly the reasoning TODO #237 and #238 were filed to undo, so it gets a deliberate review of its own: TODO #243.  `rand` is untouched by #242 and its verdict stands.
...
**§11.24.7 A coverage gap found while checking the above.**  `rand` has `CliTest/test_rand.sh` (20 assertions, including a `HERRADURA HDRBG STATE` PEM round-trip).  `fpe` and `twk` had no `CliTest` script at all — no CLI-level test, and no cross-language interop test, in a repository whose CI otherwise runs a four-language compatibility matrix over every other protocol family.  That the C, Go and Python CLIs agree byte-for-byte on both subcommands, including on the colliding value of §11.24.1, was established by hand for this analysis and was guarded by nothing.  That gap is why the §11.24.1 defect survived as long as it did.

The suite-level tests are a more mixed picture, and an earlier draft of this section overstated it.  `test_fpe_correctness` and `test_twk_correctness` exist in the C, Go, Python and Arduino harnesses.  The C harness calls the shipped `fpe_encrypt` from `herradura.h` and the Go harness calls the shipped `FpeEncrypt` from the `herradura` package, so both do exercise the real code.  The **Python** harness does not: it is standalone by design and re-implements the construction locally, so its copy could drift from the original undetected.  What is true of all of them is that the property they assert — encrypt-then-decrypt round-trip under the same derived subkey — is a tautology for any bijection, so they would have passed unchanged even with the subkey derivation removed entirely.  None of them could have caught §11.24.1.
