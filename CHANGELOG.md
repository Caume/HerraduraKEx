...
### Added
- **`BITARRAY.md` — the normative BitArray specification, and `KAT/bitarray.json`, its
  conformance oracle (TODO #314, first pass).**  One variable-width bit-string type, to be
  implemented as the same algorithm over the same representation in C, Go, Python and
  Java, replacing the three embedded bignum types (`math/big`, Python `int`,
  `java.math.BigInteger`) the suite currently delegates to.  Big-endian octet string with
  the width carried **with** the value — never passed alongside it, which is what makes
  TODO #313's "two ports ignored the declared width" unrepresentable rather than merely
  discouraged.  378 pinned cases at widths 32/64/128/256 covering every operation in §4
  plus the mixed-width, invalid-width and no-polynomial cases; error cases pin an expected
  **error code**, not a value, so a port that fails for the right reason is
  distinguishable from one that fails for the wrong one.
- `KAT/generate_bitarray_kat.py` — the reference implementation and vector generator.
  `--check` (currency) is gating and runs in `CliTest/test_kat_vectors.sh`; `--report`
  (per-port conformance) deliberately is **not** a gate, because an unconverted port is
  expected to diverge and there is no allow-list.

### Fixed
- Nothing.  **No shipped port was touched**: the suite, the four CLIs and every existing
  test are byte-for-byte unchanged.  Passes 2-6 (`BITARRAY.md` §8) convert one port per
  release; the defects below are recorded, not yet repaired.

### Changed
- **The truncation rule is decided — HIGH bits, the big-endian prefix** (`BITARRAY.md`
  §4.4), settling the split TODO #313 could only refuse around.  Python's `rnl_kdf_seed`
  and C's declared narrow constants (`_RNL_KDF_DC_32`, `_RNL_KDF_DC_64`) already say high
  bits; Go's rule was **measured**, not read — `ROL(k, n/8) XOR (DC_256 & (2^n - 1))`
  reproduces Go's output exactly at n = 32, 64 and 128.  One truncation, in one place.
- **TODO #314's versioning question is settled and the answer is not MAJOR.**
  `bindings/ffi/herradura_shim.h` mentions `BitArray` **zero times** (the FFI ABI is flat
  `uint8_t[KEYBYTES]` buffers by design), `docs/examples/c/hello_herradura.c` calls `ba_*`
  only and compiles unchanged, and `herradura.h` is header-only so no ABI boundary for the
  type exists.  Passes 2-5 are MINOR at most.

### Notes
- **Two variable-width ports, one XOR, two different silent wrong answers** — measured,
  and the concrete form of TODO #314's reason 2.  A 64-bit array XORed with a 256-bit one
  yields, in Go, a value declaring `size = 64` whose `Val` is 201 bits, which `Bytes()`
  renders as **eight zero octets**: it copies nothing when the value overflows the declared
  width, so an over-wide intermediate reads back as zero with no error and no panic.  The
  same expression in Python masks to the left operand and **discards the right one
  entirely**.  C and Java cannot pose the question.  `BITARRAY.md` §3 makes both
  `E_MIXED_WIDTH`.
- **The reference is behaviour-preserving, across three ports.**  It reproduces the shipped
  C suite at n = 256, the shipped Go package at n = 256, and the shipped Python suite at
  32/64/128/256, on xor / rol / ror / fscx / fscx_revolve / gf_mul / rnl_kdf_seed /
  popcount — zero mismatches.  The contract is the existing agreed behaviour with the width
  rules made explicit, not a redesign.
- **Currency is not coverage, and the file says so.**  A vector generated by the reference
  and checked only by the reference proves that nobody edited it and nothing more; it
  becomes load-bearing at the first port and decisive at the second.  Both controls were
  verified to fire: a one-field edit makes `--check` exit 1, and flipping the reference's
  truncation to low bits flips the report's `rnl_kdf_seed` row to `DIVERGES`.
...
`SecurityProofsCode/` is ~80 more files restating the same parameters and verdicts — in prose
**and in live constants** — outside every checker, and advertised in CLAUDE.md as things to
run. Found by surveying all 33 exit-status-gating scripts at head: 17 pass, 15 exceed a 75 s
probe (documented long-runners, not failures), and **one fails**.

**`qcmdpc_parameter_selection.py` was failing at head, exit 1, three gates at once — and it
is the script that chose the deployed QC-MDPC parameters.**

```
§1: deployed instance no longer costs below 2^64
§5: BIKE L1 threshold no longer fails at d=15 (0 fails)
§6: rejection rates 0.0200/0.0200 no longer straddle 1%
```

**The cause is #276 succeeding.** Every "before" measurement — §1's cost of the instance
under criticism, §5's converse control, §6's rejection-rate comparison — read the set under
criticism from `_QCMDPC_R/_D/_T`. That worked until #276's recommendation was **adopted**, at
which point `dep == BIKE[128]`: §1 printed that BIKE-128 is worth `2^136` *and* that "a
desktop reaches it" in one breath, §5's control ran at `d=71` where the rule it exists to
break works fine, and §6 compared one instance against itself. A before/after argument cannot
read "before" from the present tense. Fixed with an explicit `RETIRED = (523, 15, 18)`
literal, a §0 frame guard that speaks up if the suite ever carries neither set, and gates
renamed to say which side they mean. Same class as v7.0.1's row-weight literal and #285's own
leg 1 — a parameter frozen into something that does not own it — **inverted**: here a
*variable* sat where a literal was needed.

Two further corrections in that file. §7's "one blocker" — multi-second Python decapsulation —
was closed by v6.7.3's bit-sliced rewrite, so the section says so instead of demanding it, and
its ladder is re-based on the retired set rather than timing the same instance twice at 1x.
And §6's premise that the weak-key cliff *could not* be measured at these parameters is
**withdrawn**: #285 §4 measured it at multiplicity 31→32. The retry-budget route selects the
same `MAX_MULT = 6`, so the constant stands with two independent justifications instead of one
and a contradiction.

**`hkex_rnl_failure_rate.py` printed a security table in which every row was wrong in the
unsafe direction.** Executed at head:

| `n` | the script printed | published (#216, #223) | overstated |
|---|---|---|---|
| 256 | 110 / 100, labelled **"Current (deployed)"** | ~32 / ~29 | 3.4x |
| 512 | 220 / 200, `✓ ≥128 classical+quantum ★` | ~87 | 2.5x |
| 1024 | 440 / 400, labelled **"(reference)"** | ~206 / ~187 | 2.1x |

`RNL_N` is 1024 since #223/v2.7.19, so the deployed ring was a "reference" while a retired one
was "deployed"; the `~105–115` anchor was retracted by #216; `_BASELINE_CL = 110` was a live
constant feeding `cl = _BASELINE_CL * (n / _BASELINE_N)`, not a comment; and `chosen_n = 512`
starred the set #216 demoted, then fed §7. **The projection is withdrawn, not re-anchored**,
and that is the judgement this item had to make: re-pointing the same model at ~32 would still
assert bits proportional to `n`, which #216's own direct figures refute (32 → 206 across
256 → 1024 is 6.4x for 4x), and #223 separately rejected n=768 on a ring-structure ground —
`x^768+1` CRT-splits over Z — that no scaling model can see. A surrogate for a computation
nobody could run has no purpose once it has been run. §7 now verifies reconciliation at the
deployed ring instead of at the declined candidate.

**`qcmdpc_bgf_failure_rate.py`**, the piece #285 recorded rather than scoped: its docstring
called `r=523, d=15, t=18, nb_iter=20` "the suite's current toy parameters", and its claim to
close the *DFR never measured* gap of #183/#186 is now unachievable. Withdrawn rather than
repaired — no trial count reaches the rate — so it reports a one-sided upper bound, never
prints `Measured DFR: 0.000000` (which reads as a result and is not one), and its 2000-trial
default drops to 400.

**`hkex_nl_verification.py`** labelled `n=32` and `n=256` "Deployed params"; both are retired
for HKEX-RNL, and n=1024 is not added because the check is a negacyclic Gauss-Jordan costing
~64x the n=256 row.

**The mechanism: check B′′, and a design error worth recording.** A blanket census over
`SecurityProofsCode/` would be almost all exemptions — retired parameters are several scripts'
*subject* — so `check_docs_consistency.py` gains a narrower check: a phrase asserting
**currency** next to a number that must equal the header constant. The first cut resolved bare
letters globally and **fired 35 times, 33 of them on correct sentences**, because `n = 256` is
`KEYBITS` in a dozen scripts and `r = 64` is a round count in two more — a letter does not
identify the protocol. Requiring the window to also name a protocol **family** takes it to one
finding, and that one was a true positive: `hkex_rnl_lattice_2026.py` still said "SECURITY.md
*currently* puts HKEX-RNL (n=256) at ~105", stale framing in the very file that corrected it.
Both directions verified by injection, and exemptions must match or fail.

**CI.** #285 added the first coverage of any self-gating analysis script and covered only one,
on runtime grounds — which is exactly why the failure above went unseen. `native-python` now
also runs `qcmdpc_parameter_selection.py --quick` (~58 s measured). `qcmdpc_bgf_variants.py`
stays out at ~11 min. The 15 long-runners remain uncovered and are recorded as such rather
than left implied.
...
### Added
- **TODO #230: the S-box question, answered by a theorem that subsumes it.** The item
  asked whether TODO #224's negative result extends to a substitution layer. It does —
  but not by extension. #224's proof quantifies over *additive* injections that reach
  the session key through the accumulator `L = xor_j M^(r-1-j).u_j`, and an S-box does
  not factor out of the iteration at all, so #224 is **silent** on it rather than
  dispositive. Measured: #224's class is exactly the step functions with
  `G(A,B) xor G(A',B)` independent of `B`, which additive injections satisfy on 100% of
  random triples and a nibblewise S-box fails on 100%.
- **The characterization theorem.** With the public phase iterating `F_B`, the
  derivation phase `G_B`, and `i + r = n`, HKEX-style agreement holds **iff** the
  composite `G_B^r . F_B2^i` is the translation `x -> x xor d(B,B2)`. Four further
  lines force the whole key-dependence of the `i`-fold iterate into an input XOR
  translation — `{F_B^i}` lies in one coset of the translation group — and yield
  `sk = Psi^{-1}(C) xor Psi^{-1}(C2) xor c`. Corollary, the universal attack:
  `sk = G_B0^r(C) xor G_B0^r(C2) xor c` with `c = G_B0^r(F_B0^i(0))`, for any `B0` Eve
  likes, at `2r + i` applications of the public step function. No private value, no
  linearity, no assumption about the step function.
- **It subsumes the existing breaks.** Verified at n=32/64/128/256, 300 sessions each:
  agreement 300/300, composite-is-a-translation 50/50, attack 300/300, and its output
  equals `hkex_classical_break.py`'s `S_(r+1).(C xor C2)` 300/300 — without ever
  evaluating `M` as a linear map. TODO #210, `hkex_nonce_impossibility.py` and TODO
  #224 are the special cases `Psi^{-1} = M^r`, the nonce as a public step parameter,
  and the additive injection.
- **A linear S-box halves TODO #210's leak, and 64 is a floor.** `L` linear makes the
  step `(L.M).(A xor B)`; with `Y = L.M + I`, agreement is equivalent to `Y^(n-1) = 0`
  and `co-rank(T_i) = dim ker Y^(2^v2(i) - 1)`. This identifies the factor 2 in #210's
  closed form as exactly `v(M+I)` — a property of that `M`, not of the construction. At
  n=256, i=64 the co-rank drops **126 -> 64**. It cannot go lower: the unconstrained
  optimum of 63 (regular nilpotent `Y`) has `Y^(n-1) != 0` and so *fails agreement*, and
  `dim ker Y^(i-1) = sum_j min(lambda_j, i-1)` is minimized by the fewest, largest
  blocks — so 64 is proved, not merely the best found. Never 0, since `Y` is nilpotent.
- **No reconciliation branch.** The discriminator this item required was the distance
  *distribution*, not the pass/fail rate — HKEX-RNL also fails exact agreement and is
  rescued by Peikert reconciliation, which needs a small Hamming distance. A single
  transposition inside one 4-bit box, the smallest departure from linearity that exists,
  already saturates the `n/2` baseline (32.3 of 64, 64.3 of 128) and is
  indistinguishable from the PRESENT S-box. Unrelated, not noisy.
- **The item's own conjecture was false, and the theorem did not need it.** #230
  conjectured that an agreeing step function must be affine over an abelian group. For
  `G(A,B) = S(A xor B)` with `i = 1` the characterization collapses to
  `(S.tau_B)^n = tau_c`, cheap enough for genuinely exhaustive sweeps: all permutations
  at n=2 and n=3, all 322 560 affine permutations at n=4 (25 216 agree), and all
  3 025 920 boxes one transposition off an agreeing affine box — which yield **15 360
  distinct non-affine permutations admitting full agreement**, confirmed over all 65 536
  tuples for a 40-box sample. The smallest is the identity with 0 and 1 swapped. The
  universal attack wins on every one. Affineness was the wrong invariant; the
  translation-coset condition is the right one.
- Scope stated explicitly: the theorem constrains the HKEX template — two parties, one
  round each, agreement by exact equality. It says nothing about multi-round protocols,
  about constructions approximate by design (HKEX-RNL, where reconciliation is
  specified rather than a rescue), or about hardness living outside the step function.
- New `SecurityProofsCode/sbox_kex_extension.py` and SecurityProofs-7.md §11.22. Part 7
  needed no split (653 of ~750 expressions). **No `--algo` tag, PEM boundary label or
  `spec/` entry**, and no wire format changed; the co-rank result is a finding about
  HSKE/HPKE that would need its own item before anything ships.
...
### Fixed
- **Corrected the NL-FSCX v2 affine weak-key density figures (TODO #168 follow-up).** The
  v1.9.140 write-up quoted the delta=0 class density as "the class density", omitting the
  second affine class delta=2^(n-1), which is non-empty exactly when 8 | n. A completed
  exhaustive count over the full key space at n <= 32 gives the true totals: n=16: 256
  keys (2^-8.0); n=20: 512 (2^-11.0); n=24: 3072 (2^-12.4); n=28: 8192 (2^-15.0); n=32:
  40960 (2^-16.7). The n=32 figure relevant to the assembly/Arduino targets is therefore
  2^-16.7 (about 1 key in 105,000), not the 2^-17 (1 in 131,000) previously stated. The
  deployed n=256 figure is unchanged at ~2^-129, since the delta=0 class (2^127) dominates
  the delta=2^255 class (~2^97) there. No code change -- the guard already rejected both
  classes; only the published density figures were imprecise.
...
- `SecurityProofs-2.md` §11.19.2 records that the class density scales with word size --
  the delta=0 class is every K divisible by 2^ceil((n+1)/2) (density 2^-ceil((n+2)/2)),
  and the delta=2^(n-1) class is additionally non-empty whenever 8 | n. Exhaustive counts
  over the full key space at n <= 32 give a total affine density of 2^-16.7 at n=32
  (about 1 key in 105,000) against ~2^-129 at n=256, where the delta=0 class dominates.
  So the assembly/Arduino targets (which implement NL-FSCX v2 on 32-bit operands) are
  substantially more exposed than the 256-bit deployment. Those targets are
