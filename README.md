# Herradura Cryptographic Suite (v2.7.17)
...
7. **HKEX-RNL** — Ring-LWR key exchange (lattice-based, but demo-only at the deployed parameters — see `SECURITY.md`): shared $m_\text{blind}$ in $\mathcal R_q = (\mathbb{Z}/q\mathbb{Z})[x]/(x^n+1)$; parties derive $C = \lfloor m_\text{blind} \cdot s \rceil_p$; agreement $K = \lfloor s \cdot \text{lift}(C_2) \rceil_{p'}$
