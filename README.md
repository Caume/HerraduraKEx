# Herradura Cryptographic Suite (v7.0.11)
...
7. **HKEX-RNL** — Ring-LWR key exchange (conjectured quantum-resistant; ring dimension $n=1024$ since v2.7.19 — older $n=256$ keys are insecure, see `MIGRATING.md`): shared $m_\text{blind}$ in $\mathcal R_q = (\mathbb{Z}/q\mathbb{Z})[x]/(x^n+1)$; parties derive $C = \lfloor m_\text{blind} \cdot s \rceil_p$; agreement $K = \lfloor s \cdot \text{lift}(C_2) \rceil_{p'}$
...
SecurityProofs-5.md                                 — formal analysis §11.8.3–§11.8.10 (PQ signature
...
SecurityProofs-8.md                                 — formal analysis §11.34–§11.36 (NL-FSCX v3
