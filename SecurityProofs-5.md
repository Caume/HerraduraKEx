...
**Every measurement in this subsection was taken at the parameters this protocol carried when TODO #218 ran — `r = 523`, `d = 15`, `t = 18`, `NB_ITER = 20`, which the source called toy parameters and which they were.  TODO #276 replaced them with BIKE-128's `r = 12323`, `d = 71`, `t = 134`, `NB_ITER = 5`, so the numbers below are the RECORD OF WHY THAT CHANGE HAPPENED and not a description of what ships.**  What re-measures at the deployed set, what provably cannot, and what the re-pointing corrected are in *Re-pointed at the deployed parameters* at the end of this subsection (TODO #285); read the two together.  (This section is written with code spans rather than math spans throughout: Part 4 is close to GitHub's ~750-expression KaTeX ceiling, cf. TODO #220.)
...
As originally analysed, the deployed key generation screened none of this: `qcmdpc_keygen` retried only when `h0` was non-invertible — in C, Go, and Python alike — with no spectrum test, no multiplicity bound, and no weak-key rejection of any kind.  A small fraction of users drew a materially worse key with no signal that anything was unusual, and a *supplied* key could be arbitrarily bad.  **TODO #235 Part 1 closed the first half of that**; see *What TODO #235 changed* below.  The second half stands: the PEM decode path still does not check an imported private key.
...
**Why the oracle was free.**  The standard defence is implicit rejection — on decoding failure return a pseudorandom key derived from a secret and the ciphertext, so success and failure are indistinguishable.  BIKE does exactly this, which is why its DFR argument is about IND-CCA2 rather than about user experience.  The KEM as originally analysed did the opposite at every layer: `qcmdpc_decap_bgf` returned `None` / `0` / `false`, and every CLI exited non-zero with a distinct message on `dec --algo hpke-stern-kem` and on `kex --algo hybrid-rnl-stern`.  There was no Fujisaki-Okamoto transform: decapsulation never re-encrypted to check that the ciphertext had been honestly generated, so an attacker's chosen syndrome was processed exactly like a real one.  The oracle was not a side channel to be closed — it was the documented interface.  **TODO #235 Part 2 removed it**; see the next subsection.

**What TODO #235 changed.**  The four findings above split cleanly in two.  Findings 1 and 2 — the DFR and the parameters behind it — are parameter choices that cannot be fixed without a redesign.  Findings 3 and 4 are *missing constructions* that can be added at the deployed parameters, and TODO #235 added them.  Both are verified by re-running `SecurityProofsCode/qcmdpc_dfr_weak_keys.py`, whose §4, §6 and §7 now measure the remediated behaviour rather than the original.

*Part 1 — the weak-key screen.*  `qcmdpc_keygen` now computes the distance spectrum of each candidate private polynomial and rejects any whose maximum multiplicity exceeds `QCMDPC_MAX_MULT = 5`, redrawing from the same PRF stream.  The bound is read straight off the cliff in the table above: multiplicity 6 is where the DFR first departs materially from the ordinary rate, and it is the highest value honest keygen ever reached.  The whole tail measured at 200 000 samples — the multiplicity-6 keys and every total-failure class above them — is therefore unreachable from keygen.  The cost is about 0.02% of extra draws, paid *before* the polynomial inversion, which is the expensive half of a candidate.  The screen is identical in C (`qcmdpc_key_is_strong`), Go (`QcMdpcKeyIsStrong`), Python (`qcmdpc_key_is_strong`) and Java (`qcmdpcKeyIsStrong`), and because it rejects within the PRF stream rather than reseeding, deterministic keygen from a fixed seed still agrees byte-for-byte across all four.  What it does not do is screen an *imported* key: the predicate is exported in all four languages but no PEM reader calls it, so a supplied arithmetic-progression key is still accepted and still fails every decapsulation.  That is a self-inflicted denial of service rather than a confidentiality break, which is why it is recorded here rather than filed as a blocker.

*Part 2 — the FO transform with implicit rejection.*  Decapsulation no longer has a failure path.  It decodes, checks rigidity, and returns `HFSCX-256-DS(0x10, e0 || e1 || C)` on success or `HFSCX-256-DS(0x11, z || C)` on any failure, where `z = HFSCX-256-DS(0x12, h0 || h1)` is a secret derived from the private key.  An attacker sees a 32-byte key either way.

Three details of that construction are worth stating, because each could have been done wrong.

*The rigidity check reduces to a weight check, exactly.*  The textbook FO step is "re-encrypt the recovered error and compare to the ciphertext".  Here that is redundant with what the decoder already establishes: `qcmdpc_bgf_decode` reports success only when its running syndrome reaches zero, which is the statement `e0·h0 + e1·h1 == C·h0`, and `h0` is invertible mod `x^r - 1` by construction, so that holds if and only if `e0 + e1·h_pub == C` — the re-encryption identity itself.  What decoder success does *not* establish is that the recovered error has the weight an honest encapsulation samples, so `wt(e0) + wt(e1) == t` is the one thing checked explicitly.  A useful consequence: decapsulation needs no access to `h_pub`, so the private key format is untouched.

*Every FO hash is domain-separated.*  HFSCX-256 is Merkle-Damgard, so length extension applies, and FO re-encryption hashes attacker-influenced data.  All three hashes use the domain-separated variant of TODO #93 with tags `0x10` / `0x11` / `0x12` allocated in the same namespace as the existing `0x01` / `0x02` / `0x03`, and the ideal-random-function argument the FO proof needs is the one supplied by TODO #215 (§11.9).  The ciphertext is bound into the session key on both paths, so a derived key is only ever valid for the syndrome it came from.

*`z` is derived, not stored.*  FO requires `z` to be secret and unpredictable to the attacker, not statistically independent of the key, so deriving it from the private polynomials is sound and avoids adding a PEM field — the `HERRADURA QC-MDPC` private-key wire format is unchanged and old private keys keep working.  It is taken from `h0 || h1` rather than from the support arrays because the polynomials are canonical: support order is an artefact of the sampling loop and does not survive a PEM round-trip, so an imported key would otherwise derive a different `z` from the one that generated it.

*What did not change, and what it costs.*  The decoder's DFR is exactly as measured, and so is the GJS statistic in the table above: that statistic is a property of the decoder, and it was the *signalling* that made it observable.  Removing the signal removes the attack without touching the failure rate.  Two prices are paid for that.  First, a DFR event is now silent — it surfaces as a session key the two parties disagree on rather than an error, which is why `CliTest/lib_dfr.sh` was rewritten to retry on an output mismatch instead of on an error message, and why a test that only checks the exit status of `dec` no longer tests the KEM at all.  Second, this is protocol-level indistinguishability only: neither the decoder nor the branch selecting the rejection key is constant time, so a local attacker with a timing channel can still separate the two cases.  At these parameters that is not the binding constraint, and it is not claimed to be closed.
...
**Re-pointed at the deployed parameters (TODO #285).**  Everything above is a measurement at `r = 523`, `d = 15`, `t = 18`.  TODO #276 replaced that set, and `SecurityProofsCode/qcmdpc_dfr_weak_keys.py` — the script backing this subsection, and the recorded acceptance oracle for TODO #235 as well — then **exited 1 in its own §1** for two releases, because its private bit-sliced decoder held the unsatisfied-parity counters in four fixed bitplanes and `d = 71` saturates them.  Re-pointing it splits these findings cleanly in two, and the line of the split is the result: **§1 and §2's subject is the SIZE of the failure rate, §4 and §5's is its SHAPE, and only the second survives a move to parameters whose DFR is unmeasurable.**

*The twin decoder is deleted rather than widened.*  It existed because the shipped decoder was per-position and roughly 40x too slow for these sample sizes; #276 made `qcmdpc_bgf_decode` itself bit-sliced, at which point the copy bought nothing and had diverged twice over — four bitplanes, and the pre-#276 threshold schedule, a constant in `d` where the deployed rule is affine in the *syndrome weight*.  Every section now calls the function that ships, and §1 pins that against a **per-position reference** instead: `20/20` and `6/6` identical outputs at `r = 9600` and at the deployed `r`, costing `7.3 ms` against `7157 ms` and `5.4 ms` against `6083 ms` respectively.  Note that the retired parameters are not a usable cross-check instance for this decoder at all — BIKE's threshold floor of `36` unsatisfied checks is unreachable by a row of weight `15`, the failure mode §11.8.9 measures at 150 out of 150 — so both instances carry the deployed `d`.

*The DFR is no longer a measurable quantity, and that is what the change bought.*  3000 round trips with a fresh key each time give **zero failures**, i.e. a 95% upper bound of `0.123%` = `2^-9.7` and nothing sharper at any sample size a simulation can reach: `2^-128` needs about `2^128` decapsulations at `74 ms` each.  The deployed DFR claim is therefore **inherited** from BIKE's published analysis of exactly these `(r, d, t)`, not established here — the position `SECURITY.md`'s row already takes, and half of why the protocol stays demo-only.  The `2^-8.6` above is not superseded as a *measurement*; it is the measurement that justified moving off the parameters it describes.

*What is still measurable about the curve, and which way it errs.*  §3 inverts: hold the deployed `d` and `t`, and come **down** from `r = 12323` until the waterfall appears.  It does appear, at 78-80% of the deployed `r`, which is what makes the section possible.

| `r` | trials | failures | DFR | `log2` DFR |
|---|---|---|---|---|
| 9600 | 400 | 302 | 75.5% | -0.41 |
| 9700 | 600 | 190 | 31.67% | -1.66 |
| 9800 | 1200 | 74 | 6.17% | -4.02 |
| 9900 | 2600 | 11 | 0.423% | -7.88 |

The fit is `log2(DFR) = -0.02480·r + 238.3` (R² = 0.947), about `40` extra bits of `r` per bit of DFR, extrapolating to **`2^-67` at the deployed `r`**.  The **direction** of that extrapolation's error is measured rather than assumed, which the original section could not do from a single point: the successive secant slopes are `-0.0125`, `-0.0236`, `-0.0387`, so `log2(DFR)` bends downward *faster* than linear across the measured range and the straight line lies **above** the true curve to the right of it.  `2^-67` is therefore a conservative upper bound on the waterfall's continuation — the direction Sendrier-Vasseur's methodology claims — and this **corrects the reading above**, which argued the opposite sign on the strength of an error floor flattening the curve at large `r`.  Both effects are real; which dominates above `r = 9900` is not settled by anything measurable here, so `2^-67` bounds the waterfall's continuation and is not a proof about the deployed instance.  It is `61` bits from the target, and BIKE's own `2^-128` is an extrapolation too.

*The weak-key axis survives intact, and re-derives a constant #276 could not.*  A weak key does not fail *rarely* — it fails near 100% of the time — so nothing in §4 needs to resolve a probability below `1/n`, and it measures at the deployed parameters directly.  §11.8.9 records that `QCMDPC_MAX_MULT` "cannot be re-derived the way it was derived", and that is true of the *method* — resolving DFR differences among ordinary keys — but not of the constant.  Dialling multiplicity up by construction (part of `h0`'s support in an arithmetic progression, the rest random) puts the cliff at **multiplicity 31 to 32** where the retired set had it at **6 to 7** — 200 trials per point, at the deployed `r`:

| max spectrum multiplicity | AP positions | DFR |
|---|---|---|
| 4 | 2 | 0/200 |
| 7 | 8 | 0/200 |
| 15 | 16 | 0/200 |
...
One detail the table makes visible and the retired-parameter version could not: the two `31` rows differ, so **multiplicity is not the whole statistic** at this width — the length of the progression carrying it matters too.  That does not weaken the conclusion, which only needs the cliff to be far above the bound, but it does mean the cliff is a band rather than a threshold.  So the deployed bound of `6` is conservative by about `5x` rather than tuned to an edge, and **the tail it cuts was never near the cliff** — a change from the retired set, where multiplicity 6 was both the highest value honest keygen reached and worth about ten times the average DFR.  What keygen emits at `d = 71`, `r = 12323` moved too: over 100 000 sampled polynomials the mode is multiplicity `4` (72.2%), then `5` (24.2%), `3` (1.9%), `6` (1.67%), `7` (0.088%) and `8` (0.003%), so the screen rejects `0.091%` of polynomials and about one key draw in 550 — still paid before the inversion, still close to free.  (Read against §11.8.9's figure for the *old* bound of 5, which rejects everything above 5: `1.76%` of polynomials here, the `1.6%` that section records.)

*The GJS mechanism survives; its observability does not.*  The distinguisher is a **difference** between two failure rates, so it needs both to be observable, and §2 shows neither is at the deployed `r`.  §5 therefore measures it at §3's reachable instance — the same substitute-instance move §11.8.10 makes for the decoder comparison — where, over 1200 chosen ciphertexts per class at the deployed `d` and `t`, the rates are `4.83%` in-spectrum against `9.92%` out, 95% intervals `[3.69%, 6.20%]` and `[8.28%, 11.75%]`: **disjoint**, a `2.05x` ratio, and in the *same direction* as the table above — failure is *less* likely when the error distance is in the key's spectrum.  On that effect size about 200 queries resolve one distance and there are `r/2` of them, so the whole spectrum is on the order of `10^6` queries, as at the retired parameters.  The mechanism is a property of bit-flipping decoding rather than of `r`, and it did not go away with the parameter change.  What did go away is any way to read it: at the deployed `r` there is no observable failure to sample, and since TODO #235 there is no signal even if there were.  Either alone blocks the attack, and they are independent — the first is a consequence of #276, the second predates it.

*Net effect on this subsection's verdict.*  Blockers 1 and 2 were measurements taken here — a `2^-8.6` DFR and an instance worth about `2^21` operations.  #276 removed the second outright and converted the first into an inherited claim, so the reason `HPKE-Stern-KEM` remains demo-only is now about **provenance** rather than parameters: what ships is a reimplementation of BIKE's decoder over a suite-specific FSCX-based PRF, whose equivalence to the analysed object rests on testing (§1 here, `CliTest/test_stern_kem.sh`, and `KAT/pem/` since TODO #284) rather than on proof.  That is a weaker claim than BIKE's and a much stronger one than the toy set supported.  Blockers 3 and 4 stay closed, and §4 upgrades 3 from inherited to measured.
